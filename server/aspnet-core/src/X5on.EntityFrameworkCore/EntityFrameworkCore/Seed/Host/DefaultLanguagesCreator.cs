...
namespace X5on.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly X5onDbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = X5onConsts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(X5onDbContext context)
        {
            _context = context;
        }
